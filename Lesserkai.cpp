...
pthread_attr_t cs; // クリティカルセクションオブジェクト
...
#endif
{
	char buf[10000];//10000文字分の文字列を確保
	while (true) {
		fgets(buf, 10000, stdin);//fgetsで一行分取り出す stdinは入力待ちだがキーボードからとは限らない
		buf[strlen(buf) - 1] = 0; // 改行コードを消す。

		if (strncmp(buf, "quit", strlen("quit")) == 0) {//strncmpは3つ目に指定した数までの文字列の比較同じなら1を返す
		...
		if (strncmp(buf, "stop", strlen("stop")) == 0) {
			isStopReceived = true;//stopの文字列を受信したら一時停止
		}
		if (strncmp(buf, "ponderhit", strlen("ponderhit")) == 0) {
			ponderhitReceiveTime = timeGetTime();//ponderhitを受信した時刻をponderhitReceiveTimeに代入
		}
		if (strncmp(buf, "gameover", strlen("gameover")) == 0) {
			isStopReceived = true; // 先読み中にgameoverが送られてきた場合、すぐ思考を打ち切る。
		}
		string commandStr = buf;//string型のcommandStrに受信した行を入れておく
...
// コマンド受信スレッドを開始する。
bool BeginThread()//ここまだ分からない2015/4/2
{
#ifdef _WIN32
...
int main()
{
	setvbuf(stdout, NULL, _IONBF, 0); // これが必要（標準出力のバッファリングを無効にする） NULLの部分をbufとかにすると指定したバッファに標準出力する関数
	setvbuf(stdin, NULL, _IONBF, 0); // これは不要かもしれないけど念のため。

	//InitializeCriticalSection(&cs); // クリティカルセクションオブジェクトの初期化。
	bool isSuccess = BeginThread(); // 受信スレッド開始。
	if (!isSuccess) {
		printf("Failed to begin thread\n");//受信スレッドが開始してなかったら異常終了
		return -1;
	}
	...
	while (true) {
		while (true) {
			...
			const char *buf = commandStr.c_str();//c_strはchar型にStringをコピー
			int len = strlen(buf);
			if (strncmp(buf, "usi", len) == 0) {//ifは0が偽だから == 0で逆にしてる
				...
				} else if (strncmp(buf, "setoption name BookFile value ", strlen("setoption name BookFile value ")) == 0) {
					bookFileStr = buf + strlen("setoption name BookFile value ");
				} else if (strncmp(buf, "setoption name UseBook value ", strlen("setoption name UseBook value ")) == 0) {
					string useBookStr = buf + strlen("setoption name UseBook value ");
					isUseJoseki = useBookStr == "true";
				}
		...
		if (joseki.get() == NULL) {
			// 定跡ファイルが指定されていて、まだそれを読んでいなかったら、そのファイルを読み込む。
			joseki.reset(new Joseki((char *)bookFileStr.c_str()));
		}
